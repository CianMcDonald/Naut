...
function getCSScoreFromMatchInfo(playersMatchesInfo: MatchV5DTOs.ParticipantDto[]){
    return playersMatchesInfo.map(match => {
        return ((match.totalMinionsKilled + match.neutralMinionsKilled) / (match.timePlayed /60))
    });
}
